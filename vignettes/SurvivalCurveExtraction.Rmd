---
title: "SurvivalCurveExtraction: R Shiny application for extracting parameter estimates from generated individual Patient Data (IPD)"
output: 
  rmarkdown::html_vignette:
    toc: true
  # Unhighlight below to produce pdf document
  # bookdown::pdf_book:
    # pandoc_args: --top-level-division=chapter
vignette: >
  %\VignetteIndexEntry{SurvivalCurveExtraction}
...
library("pacman")
p_load(here, dplyr, knitr, SurvivalCurveExtraction, png,grid)
```

# Introduction

The `SurvivalCurveExtraction` package is used in situations where researchers are interested in simulating time to event distributions and parameter estimates based on figures/graphics of previously published studies using the [Guyot et al's](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/1471-2288-12-9) method.
...
You can install the released version of SurvivalCurveExtraction from [CRAN](https://CRAN.R-project.org) with:
...
## 1. Digitizing the Survival Curve
  ...
  1. **Before** embarking on the use of the dashboard, the user is required to have the necessary data files extracted from previous studies as described in [Guyot et al](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/1471-2288-12-9). The first dataset should look like below:
...
This file can be extracted from the website [WebPlotDigitizer](https://automeris.io/WebPlotDigitizer/). In fact let's walk through one example of a study using [maintenance Olaparib in patients with newly diagnosed advanced Ovarian cancer](https://www.nejm.org/doi/full/10.1056/NEJMoa1810858). The survival curve that we're going to try to extract is the Progression Free Survival (PFS) curve for patients that were using Olaparib.

```{r}
include_graphics(paste0(here(),"/inst/app/www/moore_b_ola.png"))
```
...
```{r}
include_graphics(paste0(here(),"/inst/app/www/wpd_axes3.png"))
```
...
```{r}
include_graphics(paste0(here(),"/inst/app/www/wpd_extract.png"))
```

Once you've drawn over the curve make sure the `Foreground Color` is the appropriate color of the data points.

```{r}
include_graphics(paste0(here(),"/inst/app/www/wpd_foreground.png"))
```
...
There are several ways that one could start the dashboard application:
  ...
  1. Use [shinyapps.io](https://pharmacoecon.shinyapps.io/SurvivalCurveExtraction/) 
  2. Clone/Fork the [github repo](https://github.com/ck2136/SurvivalCurveExtraction) and run the application using [Rstudio](https://rstudio.com/products/rstudio/download/#download)
  
Since it's easier to use the web-application. I'm going to go with the server. Click to access [shinyapps.io](https://pharmacoecon.shinyapps.io/SurvivalCurveExtraction)

In the Introduction tab there is a guideline that everyone should be able to follow.

```{r}
include_graphics(paste0(here(),"/inst/app/www/main.png"))
```
...
```{r}
include_graphics(paste0(here(),"/inst/app/www/datainput.png"))
```
...
```{r}
include_graphics(paste0(here(),"/inst/app/www/parest.png"))
```

# Contact

If there are any questions please do not hesitate to contact [me](mailto:chong.kim@ucdenver.edu)

# Code of Conduct

Please note that the 'SurvivalCurveExtraction' project is released with a
