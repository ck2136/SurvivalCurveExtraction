...
This repository contains the codes necessary to produce an interactive [shiny dash board](https://pharmacoecon.shinyapps.io/SurvivalCurveExtraction/). The original [Guyot et alâ€™s](https://bmcmedresmethodol.biomedcentral.com/articles/10.1186/1471-2288-12-9) code is embedded in the dashboard. The original purpose of the document was to serve as the analysis toolkit for generating survival distribution parameters and fit statistics for health technology assessment (HTA) or cost-effectiveness analysis of pharmaceutical interventions. 
...
```{r}
df1 <- read.csv(paste0(here(),"/data/moore_b_ola.csv"), header = FALSE)
df1 %>%
  head %>%
  kable 
```
...
source(paste0(here(),"/R/monotone_check.R"))
...
```{r}
df2 <- read.csv(paste0(here(),"/data/moore_b_ola_nrisk.csv"), header = TRUE)
df2 %>%
  kable 
```
...
Since it's easier to use the web-application. I'm going to go with the server.
